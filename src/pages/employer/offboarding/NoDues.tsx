...
const mockNoDues: NoDues[] = [
  { id: '1', title: 'Clearance - Sarah Miller', user: 'Sarah Miller', date: '03/28/2026', status: 'Draft' },
  { id: '2', title: 'Clearance - Robert Brown', user: 'Robert Brown', date: '03/15/2026', status: 'Cleared' },
];
    ...
    <div className="p-8 bg-slate-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-2xl font-black text-slate-900 uppercase tracking-tight">No Dues / Clearance Certificates</h1>
          <div className="flex items-center gap-2 text-xs font-bold text-slate-400 mt-1 uppercase tracking-wider">
            <span>Dashboard</span>
            <span>-</span>
            <span>Offboarding</span>
            <span>-</span>
            <span className="text-blue-600">No Dues / Clearance Certificate</span>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          ...
          >
            <Plus size={18} />
            ADD NEW NO DUES / CLEARANCE CERTIFICATE
          </button>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-white sticky top-0 z-10">
                <h2 className="text-xl font-black text-slate-900 tracking-tight uppercase">Add New No Dues / Clearance Certificate</h2>
                <button 
                  onClick={() => setIsModalOpen(false)}
              ...
              <div className="px-6 border-b border-slate-100 flex items-center gap-8 bg-white sticky top-[73px] z-10">
                ...
                >
                  No Dues / Clearance Certificate Details
                  {activeTab === 'details' && (
                ...
                {activeTab === 'details' ? (
                  <div className="space-y-8">
                    ...
                    <div className="bg-slate-50/50 p-6 rounded-2xl border border-slate-100 space-y-6">
                      <h4 className="text-xs font-black text-slate-900 uppercase tracking-widest">Clearance Checklist</h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <button 
                          onClick={() => setChecklist(prev => ({ ...prev, it: !prev.it }))}
                          className="flex items-center gap-3 group"
                        >
                          {checklist.it ? (
                            <CheckSquare className="text-blue-600" size={20} />
                          ) : (
                            <Square className="text-slate-300 group-hover:text-slate-400 transition-colors" size={20} />
                          )}
                          <span className="text-sm font-medium text-slate-600">IT Clearance (Laptop, Email)</span>
                        </button>
                        <button 
                          onClick={() => setChecklist(prev => ({ ...prev, hr: !prev.hr }))}
                          className="flex items-center gap-3 group"
                        >
                          {checklist.hr ? (
                            <CheckSquare className="text-blue-600" size={20} />
                          ...
                          <span className="text-sm font-medium text-slate-600">HR Clearance</span>
                        </button>
                        <button 
                          onClick={() => setChecklist(prev => ({ ...prev, finance: !prev.finance }))}
                          className="flex items-center gap-3 group"
                        >
                          {checklist.finance ? (
                            <CheckSquare className="text-blue-600" size={20} />
                          ...
                          <span className="text-sm font-medium text-slate-600">Finance Clearance</span>
                        </button>
                        <button 
                          onClick={() => setChecklist(prev => ({ ...prev, admin: !prev.admin }))}
                          className="flex items-center gap-3 group"
                        >
                          {checklist.admin ? (
                            <CheckSquare className="text-blue-600" size={20} />
                          ...
                          <span className="text-sm font-medium text-slate-600">Admin Clearance</span>
                        </button>
                      </div>
                    </div>
